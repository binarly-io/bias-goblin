...
impl TryFrom<u16> for AttributeCertificateType {
    ...
    fn try_from(value: u16) -> Result<Self, Self::Error> {
        Ok(match value {
            ...
            x if x == AttributeCertificateType::EfiPkcs115 as u16 => {
                AttributeCertificateType::EfiPkcs115
            }
            x if x == AttributeCertificateType::EfiGuid as u16 => AttributeCertificateType::EfiGuid,
            _ => {
                return Err(error::Error::Malformed(
...
#[cfg(test)]
mod tests {
    use crate::pe::certificate_table::{AttributeCertificateRevision, AttributeCertificateType};
    use crate::pe::PE;

    #[test]
    fn parse_certs_table() -> Result<(), Box<dyn std::error::Error>> {
        let file = include_bytes!("../../tests/bins/efi/RealtekLan.efi");
        let pe = PE::parse(&file[..])?;

        let mut certs = pe.certificates.iter();
        let cert = certs.next().expect("certificate");

        assert_eq!(cert.length, 8560);
        assert_eq!(cert.revision, AttributeCertificateRevision::Revision2_0);
        assert_eq!(
            cert.certificate_type,
            AttributeCertificateType::PkcsSignedData
        );

        let file = include_bytes!("../../tests/bins/efi/MultiCerts.efi");
        let pe = PE::parse(&file[..])?;

        for cert in pe.certificates {
            assert_eq!(cert.length, 1684);
            assert_eq!(cert.revision, AttributeCertificateRevision::Revision2_0);
            assert_eq!(
                cert.certificate_type,
                AttributeCertificateType::PkcsSignedData
